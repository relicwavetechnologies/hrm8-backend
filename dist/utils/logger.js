...
class Logger {
    ...
    http(method, path, statusCode, duration) {
        ...
        if (statusCode && statusCode >= 400)
            ...
            color = '\x1b[33m'; // Yellow for redirects
...
exports.logger = {
    info: (message, meta) => Logger.create('app').info(message, meta),
    warn: (message, meta) => Logger.create('app').warn(message, meta),
    error: (message, meta) => Logger.create('app').error(message, meta),
    debug: (message, meta) => Logger.create('app').debug(message, meta),
};
