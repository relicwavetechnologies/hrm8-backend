...
exports.authenticateConsultantStrict = authenticateConsultantStrict;
const prisma_1 = require("../utils/prisma");
async function authenticateConsultant(req, res, next) {
    try {
        ...
        if (!token && req.cookies && req.cookies.hrm8SessionId) {
            ...
            const adminSession = await prisma_1.prisma.session.findUnique({
                where: { session_id: adminToken },
                include: { user: true }
            });
            if (adminSession && adminSession.expires_at >= new Date()) {
                // Find a real consultant to masquerade as
                const realConsultant = await prisma_1.prisma.consultant.findFirst({
                    where: { status: 'ACTIVE' }
                });
                if (realConsultant) {
                    req.consultant = {
        ...
        const session = token ? await prisma_1.prisma.consultantSession.findUnique({
 ...
 */
async function authenticateConsultantStrict(req, res, next) {
    const authHeader = req.headers.authorization;
    let token = '';
    if (authHeader?.startsWith('Bearer '))
        token = authHeader.split(' ')[1];
    else if (req.cookies?.consultantToken)
        token = req.cookies.consultantToken;
    if (!token) {
