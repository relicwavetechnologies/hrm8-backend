...
const getJobStatusArgs = zod_1.z.object({
    jobQuery: textQuerySchema.describe('Job ID, job code, job title, or company name/domain'),
});
const getCandidateStatusArgs = zod_1.z.object({
    candidateQuery: textQuerySchema.describe('Candidate ID, email, or full name'),
    jobQuery: zod_1.z.string().trim().min(2).max(120).optional(),
});
const getJobPipelineSummaryArgs = zod_1.z.object({
    jobQuery: textQuerySchema.describe('Job ID, job code, or job title'),
});
const getCompanyHiringOverviewArgs = zod_1.z.object({
    companyId: zod_1.z.string().uuid().optional().describe('Optional company UUID, allowed only for HRM8 global admin'),
    companyQuery: zod_1.z.string().trim().min(2).max(120).optional().describe('Company name/domain/id query'),
});
const searchEntitiesArgs = zod_1.z.object({
    query: zod_1.z.string().trim().min(2).max(120),
    entityType: zod_1.z.enum(['job', 'candidate', 'company', 'application']).optional(),
    limit: zod_1.z.number().int().min(1).max(10).optional(),
});
const getApplicationTimelineArgs = zod_1.z
    .object({
    applicationId: zod_1.z.string().uuid().optional(),
    candidateQuery: zod_1.z.string().trim().min(2).max(120).optional(),
    jobQuery: zod_1.z.string().trim().min(2).max(120).optional(),
})
...
function buildCompanyTokenAndFilter(tokens) {
    if (!tokens.length)
        return null;
    return {
...
function isGlobalHrm8(actor) {
    return actor.actorType === 'HRM8_USER' && actor.role === 'GLOBAL_ADMIN';
}
function getRegionScope(actor) {
    if (actor.actorType !== 'HRM8_USER')
        return null;
    if (isGlobalHrm8(actor))
        return null;
    return actor.assignedRegionIds ?? [];
}
function ensureNonEmptyRegionScope(actor) {
...
const getJobPipelineSummaryTool = {
    ...
    run: async (input, actor) => {
        ...
        const job = await prisma_1.prisma.job.findFirst({ where: jobWhere, select: { id: true, title: true, job_code: true } });
        if (!job) {
...
const getCompanyHiringOverviewTool = {
    ...
    run: async (input, actor) => {
        ...
        if (actor.actorType === 'COMPANY_USER') {
            ...
            const [company, jobsTotal, applicationsTotal, jobStatusGroups, appStatusGroups, latestJobs] = await Promise.all([
                ...
                prisma_1.prisma.job.count({ where: { company_id: effectiveCompanyId } }),
                prisma_1.prisma.application.count({ where: { job: { is: { company_id: effectiveCompanyId } } } }),
        ...
        const matchedCompanies = await prisma_1.prisma.company.findMany({
            where: {
                OR: [
                    { id: resolvedQuery },
        ...
        const effectiveMatchedCompanies = matchedCompanies.length
            ...
            : fallbackCompanyIdsFromJobs.length
                ? await prisma_1.prisma.company.findMany({
...
const getApplicationTimelineTool = {
...
function getToolByName(name) {
    return exports.assistantTools.find((tool) => tool.name === name);
}
