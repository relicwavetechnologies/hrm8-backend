...
const job_allocation_repository_1 = require("../hrm8/job-allocation.repository");
const session_1 = require("../../utils/session");
const password_1 = require("../../utils/password");
class ConsultantService extends service_1.BaseService {
    constructor() {
        super();
    ...
    async login(data) {
        ...
        const isValid = await (0, password_1.comparePassword)(data.password, consultant.password_hash);
        if (!isValid) {
            throw new http_exception_1.HttpException(401, 'Invalid credentials');
        }
        ...
        await prisma_1.prisma.consultantSession.create({
            data: {
    ...
    async logout(sessionId) {
        await prisma_1.prisma.consultantSession.delete({
            where: { session_id: sessionId }
        });
    }
    async getCurrentConsultant(sessionId) {
    ...
    async updateProfile(consultantId, data) {
        ...
        const updateData = Object.fromEntries(Object.entries(allowedUpdates).filter(([_, v]) => v !== undefined));
        return prisma_1.prisma.consultant.update({
            where: { id: consultantId },
            data: updateData
        });
    }
    async getAssignedJobs(consultantId, filters) {
        ...
        const assignments = await prisma_1.prisma.consultantJobAssignment.findMany({
    ...
    async getJobDetails(consultantId, jobId) {
        ...
        const teamAssignments = await prisma_1.prisma.consultantJobAssignment.findMany({
    ...
    async updateJobPipeline(consultantId, jobId, data) {
        return this.jobAllocationService.updatePipelineForConsultantJob(consultantId, jobId, {
            ...data,
            updatedBy: consultantId
        });
    }
    async submitShortlist(consultantId, jobId, candidateIds, notes) {
    ...
    async logJobActivity(consultantId, jobId, activityType, notes) {
        await this.updateJobPipeline(consultantId, jobId, {
    ...
    async getPerformanceMetrics(consultantId) {
        const consultant = await prisma_1.prisma.consultant.findUnique({
            where: { id: consultantId }
        });
        if (!consultant)
    ...
    async getDashboardAnalytics(consultantId) {
        ...
        const allAssignments = await prisma_1.prisma.consultantJobAssignment.findMany({
            where: { consultant_id: consultantId, status: 'ACTIVE' },
            select: { job_id: true }
        });
        const jobIds = allAssignments.map(a => a.job_id);
        // Group applications by stage for these jobs
        const pipelineGroups = await prisma_1.prisma.application.groupBy({
            by: ['stage'],
            where: {
                job_id: { in: jobIds },
        ...
        const recentCommissionsRaw = await prisma_1.prisma.commission.findMany({
        ...
        const totalJobs = await prisma_1.prisma.consultantJobAssignment.count({ where: { consultant_id: consultantId } });
