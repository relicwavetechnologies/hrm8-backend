...
class AlertController {
    constructor() {
         ...
         * GET /hrm8/alerts
         */
        this.getActiveAlerts = async (_req, res) => {
            try {
                // Generate real-time alerts based on system state
                const alerts = [];
                // Check for overdue invoices
                try {
                    const overdueInvoices = await prisma_1.prisma.bill.count({
                        where: {
                ...
                try {
                    const pendingSettlements = await prisma_1.prisma.settlement.count({
                        where: {
                            status: 'PENDING'
                        }
                    });
                ...
                try {
                    const pendingConversions = await prisma_1.prisma.leadConversionRequest.count({
                        where: {
                            status: 'PENDING'
                        }
                    });
                ...
                try {
                    const openJobs = await prisma_1.prisma.job.count({
                        where: {
                            status: 'OPEN'
                        }
                    });
