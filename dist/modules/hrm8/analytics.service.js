...
class AnalyticsService extends service_1.BaseService {
    constructor(analyticsRepository) {
        super();
        this.analyticsRepository = analyticsRepository;
    }
    async getOperationalStats(regionId, assignedRegionIds, options) {
    ...
    async getRegionalCompanies(regionId, status, assignedRegionIds) {
        if (assignedRegionIds && assignedRegionIds.length > 0) {
            const isAll = regionId === 'all';
            if (!isAll && !assignedRegionIds.includes(regionId)) {
                throw new http_exception_1.HttpException(403, 'Access denied to this region');
        ...
        return this.analyticsRepository.getRegionalCompanies(resolvedRegionId, status);
    }
    async getPlatformOverview(filters) {
        const startDate = filters.startDate ? new Date(filters.startDate) : undefined;
        const endDate = filters.endDate ? new Date(filters.endDate) : undefined;
        const { jobs, analytics, totalApplications, uniqueCompaniesCount } = await this.analyticsRepository.getPlatformOverview(startDate, endDate, filters.companyId, filters.regionId);
        // Aggregate metrics
    ...
    async getPlatformTrends(period = '30d', filters) {
        ...
        const { analytics, applications } = await this.analyticsRepository.getPlatformTrends(startDate, endDate, filters.companyId, filters.regionId);
        // Group by date
        const dailyData = {};
    ...
    async getTopPerformingCompanies(limit = 10, regionId) {
