...
class WithdrawalService extends service_1.BaseService {
    async getScopedWithdrawal(id, allowedRegionIds) {
    ...
    async getPendingWithdrawals(allowedRegionIds) {
    ...
    async approveWithdrawal(id, allowedRegionIds, adminId) {
        // Find
    ...
    async rejectWithdrawal(id, allowedRegionIds, adminId, reason) {
        const w = await this.getScopedWithdrawal(id, allowedRegionIds);
        if (w.status !== 'PENDING')
    ...
    async processPayment(id, notes, allowedRegionIds, adminId) {
        const w = await this.getScopedWithdrawal(id, allowedRegionIds);
        ...
        return prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (latest.status !== 'APPROVED' && latest.status !== 'PROCESSING' && latest.status !== 'COMPLETED') {
                throw new http_exception_1.HttpException(400, 'Withdrawal is no longer payable');
            }
            ...
            if (!latest.debited_from_wallet && !debitTransaction) {
                ...
                if (Number(account.balance) < latest.amount) {
                    throw new http_exception_1.HttpException(400, 'Insufficient wallet balance for withdrawal debit');
                }
            ...
            if (!debitTransaction) {
                throw new http_exception_1.HttpException(500, 'Withdrawal debit transaction missing. Cannot complete payout without wallet debit.');
            }
