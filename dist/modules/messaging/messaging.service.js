...
class MessagingService extends service_1.BaseService {
    async createConversation(participants, type = client_1.ConversationChannelType.CANDIDATE_EMPLOYER, metadata) {
    ...
    async getJobConversations(jobId, userId) {
        return await this.prisma.conversation.findMany({
            where: {
                job_id: jobId,
                participants: {
    ...
    async sendMessage(sender, conversationId, content, type = client_1.MessageContentType.TEXT, metadata) {
        ...
        if (sender.type !== 'SYSTEM') {
            ...
            if (sender.type === client_1.ParticipantType.CANDIDATE) {
            ...
            else if (sender.type === client_1.ParticipantType.EMPLOYER || sender.type === client_1.ParticipantType.CONSULTANT) {
                // HR RESTRICTION
    ...
    async joinConversation(participant, conversationId) {
