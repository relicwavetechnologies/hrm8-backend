...
/** Frontend ApplicationQuestion-compatible shape for screening questions */
export interface ScreeningQuestionOutput {
  id: string;
  type: 'short_text' | 'long_text' | 'yes_no' | 'multiple_choice' | 'dropdown' | 'number' | 'date' | 'file_upload' | 'checkbox';
  label: string;
  required: boolean;
...
export class QuestionGenerationService {
   ...
   */
  static async generateScreeningQuestions(input: GenerateScreeningQuestionsInput): Promise<ScreeningQuestionOutput[]> {
    const {
      jobTitle,
      jobDescription = '',
      companyContext = '',
...
Use a mix of question types: some short_text, some long_text for open-ended, yes_no for eligibility, multiple_choice or dropdown when fixed options make sense (e.g. years of experience, work authorization).`;
    ...
    try {
      ...
      return raw.slice(0, count).map((q: any, index: number) => {
  ...
  private static normalizeQuestionType(t: string): ScreeningQuestionOutput['type'] {
    const map: Record<string, ScreeningQuestionOutput['type']> = {
  ...
  private static fallbackScreeningQuestions(jobTitle: string, count: number): ScreeningQuestionOutput[] {
    const defaults: ScreeningQuestionOutput[] = [
