...
import { ConversationChannelType, ParticipantType, ConversationStatus } from '@prisma/client';
import { prisma } from '../../utils/prisma'; // Added for direct participant lookup if needed, though best to abstract
...
export class CandidateMessagingController extends BaseController {
  ...
  createConversation = async (req: CandidateAuthenticatedRequest, res: Response) => {
    try {
      if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
  ...
  sendMessage = async (req: CandidateAuthenticatedRequest, res: Response) => {
    try {
      ...
      for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i].sender_type !== ParticipantType.CANDIDATE && messages[i].sender_type !== ('SYSTEM' as ParticipantType)) { // Type cast SYSTEM as it might not be in ParticipantType enum yet if Prisma outdated
      ...
      for (let i = lastHrMessageIndex + 1; i < messages.length; i++) {
        if (messages[i].sender_type === ParticipantType.CANDIDATE && messages[i].sender_id === req.candidate.id) {
  ...
  archiveConversation = async (req: CandidateAuthenticatedRequest, res: Response) => {
    try {
      if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
      const conversationId = Array.isArray(req.params.conversationId) ? req.params.conversationId[0] : req.params.conversationId;
      ...
      const isParticipant = (conversation as any).participants.some((p: any) => p.participant_id === req.candidate!.id);
      if (!isParticipant) {
        return this.sendError(res, new Error('Unauthorized'), 403);
      }
  ...
  closeConversation = async (req: CandidateAuthenticatedRequest, res: Response) => {
    try {
      if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
      const conversationId = Array.isArray(req.params.conversationId) ? req.params.conversationId[0] : req.params.conversationId;
