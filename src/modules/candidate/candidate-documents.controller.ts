...
export class CandidateDocumentsController extends BaseController {
    ...
    parseResume = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
            if (!req.file) return this.sendError(res, new Error('No file uploaded'), 400);

            const parsedData = await ResumeParserService.parseResume(req.file.buffer, req.file.mimetype);
    ...
    uploadResume = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
            if (!req.file) return this.sendError(res, new Error('No file uploaded'), 400);
            ...
            let parsedData = null;
            try {
                parsedData = await ResumeParserService.parseResume(req.file.buffer, req.file.mimetype);
            } catch (parseError) {
    ...
    setDefaultResume = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
    ...
    // COVER LETTERS
    listCoverLetters = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));

            const coverLetters = await prisma.candidateCoverLetter.findMany({
                where: { candidate_id: req.candidate.id },
    ...
    createCoverLetter = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
            ...
            const coverLetter = await prisma.candidateCoverLetter.create({
                data: {
                    candidate_id: req.candidate.id,
                    title,
    ...
    updateCoverLetter = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            ...
            const existing = await prisma.candidateCoverLetter.findUnique({ where: { id } });
            ...
            const updated = await prisma.candidateCoverLetter.update({
                where: { id },
                data: updateData,
            });
    ...
    deleteCoverLetter = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            ...
            const coverLetter = await prisma.candidateCoverLetter.findUnique({ where: { id } });
            ...
            await prisma.candidateCoverLetter.delete({ where: { id } });
    ...
    // PORTFOLIO
    listPortfolioItems = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));

            const portfolioItems = await prisma.candidatePortfolio.findMany({
                where: { candidate_id: req.candidate.id },
                orderBy: { updated_at: 'desc' },
            });
    ...
    createPortfolioItem = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
            ...
            const portfolio = await prisma.candidatePortfolio.create({
                data: {
                    candidate_id: req.candidate.id,
                    title,
                    type,
    ...
    updatePortfolioItem = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
            const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;

            const existing = await prisma.candidatePortfolio.findUnique({ where: { id } });
            if (!existing || existing.candidate_id !== req.candidate.id) {
            ...
            const updated = await prisma.candidatePortfolio.update({
                where: { id },
                data: updateData,
            });
    ...
    deletePortfolioItem = async (req: CandidateAuthenticatedRequest, res: Response) => {
        try {
            if (!req.candidate) return this.sendError(res, new Error('Not authenticated'));
            const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;

            const item = await prisma.candidatePortfolio.findUnique({ where: { id } });
            ...
            await prisma.candidatePortfolio.delete({ where: { id } });
