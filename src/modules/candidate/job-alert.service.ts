...
import { Job, JobAlert, Candidate, Prisma } from '@prisma/client';
import { NotificationService } from '../notification/notification.service';
import { EmailService } from '../email/email.service';
import { Logger } from '../../utils/logger';
import { prisma } from '../../utils/prisma';

interface JobAlertCriteria {
    keywords?: string[];
    location?: string;
...
export class JobAlertService extends BaseService {
    private logger = Logger.create('job-alert');
     ...
     */
    async processJobAlerts(job: Job): Promise<void> {
     ...
     */
    private async getActiveJobAlerts(): Promise<JobAlert[]> {
     ...
     * Get candidate by ID
     */
    private async getCandidateById(candidateId: string): Promise<Candidate | null> {
     ...
     */
    private matchJobToAlert(job: Job, alert: JobAlert): number {
        const criteria = alert.criteria as JobAlertCriteria || {};
        let score = 0;
        let matches = 0;
        let totalChecks = 0;
     ...
     */
    private formatSalaryRange(job: Job): string {
