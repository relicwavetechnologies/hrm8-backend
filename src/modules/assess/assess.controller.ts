...
export class AssessController extends BaseController {
  ...
  uploadPositionDescription = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const parsedData = await this.assessService.uploadPositionDescription(req.file, req.user.companyId as string);
  ...
  getMyJobs = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const jobs = await this.assessService.getMyJobs(req.user.companyId as string);
      return this.sendSuccess(res, { jobs });
    } catch (error) {
      return this.sendError(res, error);
    }
  ...
  getCompanyBalance = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const balance = await this.assessService.getCompanyBalance(req.user.companyId as string);
  ...
  addTestCredits = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const result = await this.assessService.addTestCredits(req.user.companyId as string, amount, req.user.id);
      return this.sendSuccess(res, result);
    } catch (error) {
      return this.sendError(res, error);
    }
  ...
  getJobWithCandidates = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const job = await this.assessService.getJobWithCandidates(jobId as string, req.user.companyId as string);
  ...
  uploadCandidateCV = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const candidateData = await this.assessService.uploadCandidateCV(req.file, req.user.companyId as string);
