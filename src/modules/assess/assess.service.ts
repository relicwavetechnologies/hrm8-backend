...
export class AssessService extends BaseService {
  ...
  async uploadPositionDescription(file: Express.Multer.File, companyId: string) {
    ...
    // In real app, use AI to parse file. For now, returning basic info + URL
    return {
      url: result.url,
      fileName: file.originalname,
      // Mocked parsed data
      title: file.originalname.split('.')[0],
      description: 'Extracted description from ' + file.originalname,
      location: 'Remote',
  ...
  async uploadCandidateCV(file: Express.Multer.File, companyId: string) {
    ...
    // In real app, parse CV. For now returning mock + URL
    return {
      url: result.url,
      fileName: file.originalname,
      firstName: 'Parsed',
