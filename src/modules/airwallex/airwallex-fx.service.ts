...
const log = Logger.create('airwallex-fx');
...
export interface FxQuote {
  ...
  quoteId: string;
  validUntil: Date;
  source: 'AIRWALLEX' | 'MOCK' | 'SAME_CURRENCY';
}
...
function getMockRate(sell: string, buy: string): number {
  if (sell === buy) return 1.0;
  const direct = MOCK_RATES[sell]?.[buy];
  if (direct) return direct;
  const inverse = MOCK_RATES[buy]?.[sell];
  if (inverse) return Number((1 / inverse).toFixed(6));
...
export class AirwallexFxService {
  static async getQuote(sellCurrency: string, buyCurrency: string): Promise<FxQuote> {
  ...
  static resolveFxFields(
    sourceCurrency: string,
    payoutCurrency: string,
    sourceAmount: number,
    quote: FxQuote
  ): { payoutAmount: number; fxRate: number; fxSource: string } {
  ...
  private static getMockQuote(sell: string, buy: string): FxQuote {
  ...
  private static async getLiveQuote(sell: string, buy: string): Promise<FxQuote> {
    ...
    try {
      ...
      if (!authRes.ok) throw new Error(`Airwallex auth failed: ${authRes.status}`);
      const { token } = await authRes.json();

      const quoteRes = await fetch(
        `${baseUrl}/api/v1/fx/rates?sell_currency=${sell}&buy_currency=${buy}&amount=1&amount_currency=${sell}`,
