...
export class SettlementService extends BaseService {
    ...
    async getStats(filters: any) {
        const stats = await this.settlementRepository.getStats(filters);
        return {
            total_settled: stats.totalSettled,
            hrm8_share: stats.hrm8Share,
            licensee_share: stats.licenseeShare,
            count: stats.count
        };
    }

    async markAsPaid(id: string, data: { paymentDate: Date; paymentReference: string }) {
        const updated = await this.settlementRepository.update(id, {
            status: 'PAID',
