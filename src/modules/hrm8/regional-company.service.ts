...
type ListCompaniesOptions = CompanyAccessOptions & {
  page?: number;
  limit?: number;
  search?: string;
  status?: string;
...
export class RegionalCompanyService extends BaseService {
  private conversionContextService = new ConversionCommercialContextService();

  private normalizeDate(value: Date | string | null | undefined, fallback: Date): Date {
  ...
  private async assertCompanyAccess(
    ...
    if (options.role === 'REGIONAL_LICENSEE') {
      ...
      if (!company.region_id || !assignedRegionIds.includes(company.region_id)) {
        throw new HttpException(403, 'Access denied for this company');
      }
    }
  ...
  async listCompanies(options: ListCompaniesOptions) {
  ...
  async getById(id: string, options?: CompanyAccessOptions) {
  ...
  async getOverview(companyId: string, options: CompanyAccessOptions) {
    await this.assertCompanyAccess(companyId, options, { region_id: true });

    const [company, openJobsCount, totalJobsCount, applicationsCount, activeSubscription, wallet, commercialContext] =
      await Promise.all([
  ...
  async getActivity(companyId: string, options: ActivityOptions) {
    await this.assertCompanyAccess(companyId, options, { region_id: true });
    ...
    const [jobs, subscriptions, bills, wallet, commissions, conversionRequests, overrides] = await Promise.all([
      ...
      prisma.enterpriseOverride.findMany({
        where: { company_id: companyId },
        orderBy: { created_at: 'desc' },
        take: limit,
        select: {
  ...
  async getCompanyUsers(companyId: string, options: CompanyAccessOptions) {
    await this.assertCompanyAccess(companyId, options, { region_id: true });
  ...
  async getPricingContext(companyId: string, options: CompanyAccessOptions) {
    ...
    const [activeOverride, availablePriceBooks] = await Promise.all([
      prisma.enterpriseOverride.findFirst({
        where: {
          company_id: companyId,
          is_active: true,
    ...
    let resolvedPriceBook: any = null;
    try {
      resolvedPriceBook = await PriceBookSelectionService.getEffectivePriceBook(companyId);
    } catch (error) {
  ...
  async getPricingOverrides(companyId: string, options: CompanyAccessOptions) {
    await this.assertCompanyAccess(companyId, options, { region_id: true });

    const overrides = await prisma.enterpriseOverride.findMany({
      where: { company_id: companyId },
      orderBy: { created_at: 'desc' },
      include: {
        price_book: {
  ...
  async createPricingOverride(
    ...
    if (options.role !== 'GLOBAL_ADMIN') {
      throw new HttpException(403, 'Only global admin can create pricing overrides');
    }
    ...
    const source = await prisma.priceBook.findUnique({
    ...
    if (!source) {
      throw new HttpException(404, 'Source price book not found');
    }
    ...
    if (effectiveTo && effectiveTo <= effectiveFrom) {
      throw new HttpException(400, 'effectiveTo must be greater than effectiveFrom');
    }

    const clonedPriceBook = await prisma.$transaction(async (tx) => {
  ...
  async activatePricingOverride(
    ...
    if (options.role !== 'GLOBAL_ADMIN') {
      throw new HttpException(403, 'Only global admin can activate pricing overrides');
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!target) {
        throw new HttpException(404, 'Pricing override not found');
      }
  ...
  async deactivatePricingOverride(
    companyId: string,
    overrideId: string,
    actorId: string,
    options: CompanyAccessOptions
    ...
    if (options.role !== 'GLOBAL_ADMIN') {
      throw new HttpException(403, 'Only global admin can deactivate pricing overrides');
    }

    await this.assertCompanyAccess(companyId, options, { region_id: true });

    const override = await prisma.enterpriseOverride.findFirst({
      where: {
        id: overrideId,
        company_id: companyId,
      },
  ...
  async getCompanyJobs(companyId: string, options?: CompanyAccessOptions) {
    await this.assertCompanyAccess(companyId, options || {}, { region_id: true });
