...
export class ComplianceService extends BaseService {
    ...
    async getOverduePayouts(thresholdDays: number = 30): Promise<ComplianceAlert[]> {
    ...
    async getInactiveRegions(thresholdDays: number = 60): Promise<ComplianceAlert[]> {
    ...
    async getRevenueDeclines(thresholdPercent: number = 20): Promise<ComplianceAlert[]> {
        const now = new Date();
    ...
    async getExpiredAgreements(): Promise<ComplianceAlert[]> {
        const now = new Date();
    ...
    async getAuditHistory(entityType: string, entityId: string, limit: number) {
        return this.auditLogService.getByEntity(entityType, entityId, limit);
    }

    async getRecentAudit(limit: number) {
