...
export class TransactionController extends BaseController {
    ...
    getTransactions = async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            if (!req.user?.companyId) {
                console.error('[TransactionController] No Company ID for user');
            ...
            const transactions = await this.service.getCompanyTransactions(req.user.companyId as string, { limit, offset });
    ...
    getStats = async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            const stats = await this.service.getCompanyTransactionStats(req.user.companyId as string);
