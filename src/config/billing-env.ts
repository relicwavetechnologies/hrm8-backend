...
export type BillingProviderMode = 'mock' | 'live';

const resolveMode = (): BillingProviderMode => {
  const raw = (process.env.BILLING_PROVIDER_MODE || 'mock').toLowerCase();
  if (raw === 'live') return 'live';
  return 'mock';
};

export const BILLING_PROVIDER_MODE = resolveMode();
...
export function validateBillingEnv(): void {
  ...
  if (missing.length > 0) {
    throw new Error(
      `[billing-env] BILLING_PROVIDER_MODE=live but the following required env vars are missing: ${missing.join(', ')}`
    );
  }
}
